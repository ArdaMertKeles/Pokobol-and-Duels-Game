...
export default function Tournament({ randomTrackPlayer, randomBattleTrack, setRandomBattleTrack, playMenuMusic, bossData, setLevelUp, enemyDataMultiplier, enemyData, characterData, currentMoney, currentXp, setCurrentMoney, setMainGame, setTournamentSection, setCurrentXp, currentProjectile, currentMagicka }) {
    ...
    const [play, {stop}] = useSound(randomBattleTrack, {volume: 0.30})
    ...
    const randomEnemy = () => {
        ...
        const randomBoss = Math.floor(Math.random() * bossData.length)
        setBoss(bossData[randomBoss])
    }
    ...
    useEffect(() => {
        ...
        if(enemyCurrentHp <= 0 && killCounter === 3){
            setEnemyDeath(true)
            setEnemyAlive(false)
            setBattleLog(prevLogs => [...prevLogs, { log: `Duel is over! ${characterData.name} has won...` }])
            setButtons(false)
            let randomMoney = Math.floor((Math.random() * 300) * enemyDataMultiplier) + 150
            setAddedMoney(randomMoney)
            setCurrentMoney(prev => prev + randomMoney)
            let randomXp = Math.floor(Math.random() * 300) + 300
            setAddedXp(randomXp)
    ...
    const callBoss = () => {
        setEnemyBaseData(boss)
    }

    const gameChecker = () => {
        setPlayerCurrentHp(characterData.hp)
        setGameCheck(false)
        setEnemyAlive(true)
        setButtons(true)
    ...
    useEffect(() => {
        ...
        setEnemy({
                ...
                name: enemyBaseData.name,
                hp: enemyBaseData.hp ,
                attack: enemyBaseData.attack,
                defence: enemyBaseData.defence,
                agility: enemyBaseData.agility,
                accuracy: enemyBaseData.accuracy 
            })
        }
    }, [enemyBaseData])
    ...
    return (
        <div className="duelWrapper">
            <Log battleLog={battleLog} />
            {gameCheck && <button className="continueBtn" onClick={gameChecker}>Continue</button>}
            ...
            {result && <Result stop={stop} playMenuMusic={playMenuMusic} setLevelUp={setLevelUp} setMainGame={setMainGame} setTournamentSection={setTournamentSection} currentMoney={currentMoney} currentXp={currentXp} addedMoney={addedMoney} addedXp={addedXp} />}
            {lostResult && <LostResult stop={stop} playMenuMusic={playMenuMusic} setMainGame={setMainGame} setTournamentSection={setTournamentSection} currentMoney={currentMoney} currentXp={currentXp} addedMoney={addedMoney} addedXp={addedXp} />}
            {}
        </div>
    )
}